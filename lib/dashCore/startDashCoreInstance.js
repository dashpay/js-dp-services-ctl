 ...
 */
async function startDashCoreInstance(options) {
  const instances = await startDashCoreInstance.many(1, options);
 ...
 * @return {Promise<DashCoreInstance[]>}
 */
startDashCoreInstance.many = async function many(number, options) {
  if (number < 1) {
  ...
  for (let i = 0; i < number; i++) {
    const instance = await createDashCoreInstance(options);
