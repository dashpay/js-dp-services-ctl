...
class Insight extends DockerService {
  ...
  /**
   * Connect to another Dapi instance
   *
   * @param {Insight} dapiInstance
   * @return {Promise<void>}
   */
  async connect(dapiInstance) {
    if (!this.isInitialized()) {
      throw new Error('Instance should be started before!');
    }

    const ip = dapiInstance.getIp();
    const port = dapiInstance.options.getDashdPort();
    await this.rpcClient.addNode(`${ip}:${port}`, 'add');
  }

  /**
   * Disconnect from another Dapi instance
   *
   * @param {Insight} dapiInstance
   * @return {Promise<void>}
   */
  async disconnect(dapiInstance) {
    if (!this.isInitialized()) {
      throw new Error('Instance should be started before!');
    }
