...
const createDriveUpdateStateApi = require(
  './driveUpdateStateApi/createDriveUpdateStateApi',
);
 ...
 * @property {DriveUpdateStateApi} driveUpdateStateApi
...
startDrive.many = async function many(number, options = {}) {
  ...
  for (let i = 0; i < number; i++) {
    ...
    const driveUpdateStateApiInstance = await createDriveUpdateStateApi(driveOptions);
    await driveUpdateStateApiInstance.start();

    const instance = {
      dashCore: dashCoreInstance,
      mongoDb: mongoDbInstance,
      driveApi: driveApiInstance,
      driveUpdateStateApi: driveUpdateStateApiInstance,
      ...
      connect: async function connect(otherInstance) {
        await Promise.all([
          ...
          instance.driveUpdateStateApi.connect(otherInstance.driveUpdateStateApi),
        ]);
      },
    };
