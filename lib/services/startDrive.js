...
startDrive.many = async function many(number, options = {}) {
  ...
  for (let i = 0; i < number; i++) {
    ...
    const envs = [
      `CORE_ZMQ_PUB_HASHBLOCK=${dashCoreInstance.getZmqSockets().hashblock}`,
      `CORE_JSON_RPC_HOST=${dashCoreInstance.getIp()}`,
      `CORE_JSON_RPC_PORT=${dashCoreInstance.options.getRpcPort()}`,
      `CORE_JSON_RPC_USERNAME=${dashCoreInstance.options.getRpcUser()}`,
      `CORE_JSON_RPC_PASSWORD=${dashCoreInstance.options.getRpcPassword()}`,
      `DOCUMENT_MONGODB_URL=mongodb://${mongoDbInstance.getIp()}:${mongoDbInstance.options.getMongoPort()}`,
      'INITIAL_CORE_CHAINLOCKED_HEIGHT=1',
    ];
