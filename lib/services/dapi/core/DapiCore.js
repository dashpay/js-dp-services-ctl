...
class DapiCore extends NodeJsService {
   ...
   */
  getApiJsonRpcPort() {
    return this.options.getApiJsonRpcPort();
  }
   ...
   */
  getApiGrpcPort() {
    return this.options.getApiGrpcPort();
  }

  /**
   * Get txFilterStream gRPC port
   *
   * @return {int} port
   */
  getTxFilterStreamGrpcPort() {
    return this.options.getTxFilterStreamGrpcPort();
  }
  ...
  async initializeJsonRpc() {
    this.dapiClient = createDapiClient(
      this.getApiJsonRpcPort(),
      this.getApiGrpcPort(),
      this.getTxFilterStreamGrpcPort(),
    );
   ...
   * @return {Promise<boolean>}
   */
  async initializeApiGrpc() {
    const healthClient = new HealthCheckClient(
      `127.0.0.1:${this.getApiGrpcPort()}`,
  ...
  async initialize() {
    ...
    if (!initializedGrpc) {
      throw new Error('DAPI Grpc API is not serving');
    }
  }
}
