...
class DapiCoreOptions extends NodeJsServiceOptions {
  ...
  mergeWithDefaultOptions(...customOptions) {
    const defaultPorts = {
      ...
      platformGrpcPort: this.getRandomPort(20002, 29998),
    ...
    const defaultServiceOptions = {
      ...
      platformGrpcPort: defaultPorts.coreGrpcPort,
      txFilterStreamGrpcPort: defaultPorts.coreGrpcPort,
    ...
    customOptions[0].container.envs.push(`CORE_GRPC_PORT=${defaultPorts.coreGrpcPort}`);
    customOptions[0].container.envs.push(`PLATFORM_GRPC_PORT=${defaultPorts.platformGrpcPort}`);
   ...
   */
  getPlatformGrpcPort() {
    return this.options.platformGrpcPort;
  }
