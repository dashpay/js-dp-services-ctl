...
class NodeJsService extends DockerService {
  ...
  async prepareNodeModulesVolume() {
    ...
    const { options: { localPackageJsonFile, localPackageLockJsonFile } } = this.options;
      ...
      undefined,
      {
        HostConfig: {
          Mounts: [
            ...
            {
              Type: 'bind',
              Source: localPackageLockJsonFile || path.join(cwd, 'package-lock.json'),
              Target: '/package-lock.json',
              ReadOnly: true,
            },
            {
              Type: 'bind',
              Source: localPackageJsonFile || path.join(cwd, 'package.json'),
              Target: '/package.json',
              ReadOnly: true,
            },
          ],
