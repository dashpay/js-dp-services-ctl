...
class NodeServiceOptions extends DockerServiceOptions {
   ...
   */
  getNpmInstallCmd() {
    const { cacheNodeModules, nodeModulesPath } = this.options;

    if (!cacheNodeModules) {
      return 'npm i --production';
    }

    const copyLockFileCmd = `cp /package-lock.json ${nodeModulesPath}/`;
    ...
    // For future checks
    const installCmd = `if [ ${md5SumVolumeFile} != ${md5SumAppFile} ]; then `
          + 'npm i --production; '
          + `${copyLockFileCmd}; `
          + `touch ${nodeModulesPath}/installed.lock; `
          + 'fi';
