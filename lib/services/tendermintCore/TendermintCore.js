...
class TendermintCore extends DockerService {
  ...
  async remove() {
    ...
    try {
      ...
      if (e.statusCode !== 409 && !e.message.startsWith(
        `(HTTP code 409) conflict - remove ${volumeName}: volume is in use`,
      )) {
        throw e;
      }
    }
  ...
  async initialize() {
    ...
    while (nodeStarting) {
      try {
        ...
        if (error.message !== 'Error: read ECONNRESET'
          && error.message !== 'Error: connect ECONNREFUSED'
          && error.message !== 'Error: socket hang up') {
          throw error;
        }
  ...
  async prepareTestnetConfig(numberOfNodes, numberOfValidators) {
    ...
    try {
      ...
      await docker.run(
        imageName,
        ['testnet', `--v=${numberOfValidators}`, `--n=${numberOfNodes - numberOfValidators}`, '--hostname-prefix=node', '--o=./mytestnet'],
        undefined,
        {
          HostConfig: {
            AutoRemove: true,
