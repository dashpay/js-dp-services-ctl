...
startDapi.many = async function many(number, options = {}) {
  ...
  console.info('Starting tendermint...');
  ...
  for (let i = 0; i < number; i++) {
    ...
    // Pass JSON RPC port and gRPC API port from DapiCore to the DapiTxFilterStream service
    dapiOptions.apiJsonRpcPort = dapiCore.options.getApiJsonRpcPort();
    dapiOptions.apiGrpcPport = dapiCore.options.getApiGrpcPort();
