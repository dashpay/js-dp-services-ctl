...
async function callInParallel(services, method) {
  if (Object.keys(services).length) {
    ...
    let promises = instancesFirst.map(instance => instance[method]());
    await Promise.all(promises);
    promises = instances.map(instance => instance[method]());
    await Promise.all(promises);
  }
}
/**
...
startDapi.many = async function many(number, options = {}) {
  ...
  for (let i = 0; i < number; i++) {
    ...
    const dapiEnvs = [
      `INSIGHT_URI=http://${insightInstance.getIp()}:${insightInstance.options.getApiPort()}/insight-api-dash`,
